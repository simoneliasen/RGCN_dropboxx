...
class TensorflowOptimizer(Optimizer):
    ...
    def update_from_batch(self, processed_batch):
        ...
        upd = self.session.run(
            [self.update_function, self.loss_function, adds], feed_dict=feed_dict
        )
...
class TheanoOptimizer(Optimizer):
    ...
    def compute_update_function(self, input_params):
        raw_update = self.stack.theano_process_update_function(
            self.__parameters__, self.__loss__
        )
        self.__update__ = theano.function(
            inputs=input_params, outputs=self.__loss__, updates=raw_update
        )
