...
if "NegativeSampleRate" in general_settings:
    ns = auxilliaries.NegativeSampler(
        int(general_settings["NegativeSampleRate"]), general_settings["EntityCount"]
    )
    ...
    def t_func(x):  # horrible hack!!!
        ...
        if not encoder.needs_graph():
            ...
            graph_split_ids = np.random.choice(
                graph_batch_ids, size=split_size, replace=False
            )
            ...
            if (
                "StoreEdgeData" in encoder_settings
                and encoder_settings["StoreEdgeData"] == "Yes"
            ):
...
optimizer = build_tensorflow(
    loss, optimizer_weights, optimizer_parameters, optimizer_input
)
